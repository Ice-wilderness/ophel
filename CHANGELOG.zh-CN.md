...
- **跨站点对话搜索** — 全局搜索现在也能搜到其他已适配站点的对话，结果带来源站点图标和「站外」标注，点击后在新标签页打开。(#888)
- **Gemini Spark 对话同步** — Gemini Spark（gemini.google.com/spark）的任务对话现在可以同步进对话管理，置顶状态一并保留，点击即可打开对应对话。(#902)

### ✨ 功能优化

- **长对话与流式回复性能** — 优化 Gemini 及各站点在长对话、持续流式回复和滚动时的后台计算，减少无谓的页面测量与扫描，缓解页面卡顿。
- **导出样式** — 新增「导出样式」设置（导出弹窗中也可单次选择），简洁样式会移除导出文件中的装饰 emoji；提示词队列与导出文案现在完整跟随所选界面语言。
- **更新提示方式** — 版本更新后不再弹出全屏更新日志，改为右下角一次性提醒，面板 Logo 显示红点；悬停可预览要点，点击查看完整日志。(#887)
- **悬浮提示** — 提示靠近屏幕边缘时自动翻转，点击或按 Esc 立即关闭；列表项只在标题被截断时显示，减少遮挡相邻内容。(#885)
- **多入口站点的快捷访问** — 绑定多个域名的站点（如 Kimi 的 kimi.com 和 kimi.ai）在弹窗「快速访问」中现在只占一个格子，可切换入口并记住上次选择。(#895)
- **功能提示「适配中心」入口** — 大纲功能提示里的「适配中心」现在可点击，直接打开设置中的在线适配库。
- **顶栏拖拽提示** — 穿透模式提示改为按住顶栏片刻才显示，点击或拖动时不再闪出遮挡标题。
- **Claude 对话切 Key 体验** — 切换 Session Key 时优先仅刷新当前发起请求的标签页，不再重载其他打开的 Claude 标签页，避免打断其他对话与未保存草稿。
...
- **弹窗界面语言** — 修复扩展弹窗始终显示浏览器语言的问题，现在会跟随设置中的界面语言。(#897)
- **DeepSeek 深度思考大纲识别** — 修复 DeepSeek R1 深度思考流式输出期间，思考过程被误识别为用户提问塞入大纲导致大纲闪烁的问题。
- **大纲字数统计跨轮次虚高** — 修复 ChatGPT、Claude、Gemini、Grok、Z.ai 等站点中，大纲章节字数跨轮次累加导致字数虚高或异常为 0 的问题。
- **Grok 提示词多行与特殊字符** — 修复在 Grok 插入包含 `<`、`>`、`&` 等字符的提示词时内容被解析为 HTML 或丢失，以及多行换行分段丢失的问题。
- **提示词拖动排序** — 修复提示词库中拖动排序失效的问题，现在任意平台筛选下都可自由拖动排序，仅「最近使用」视图和搜索时保持禁用。
...
- **ChatGPT 长对话性能优化** — ChatGPT 站点设置新增三个渲染优化开关：代码块批量挂载（默认开启，打开或切换长对话时合并挂载代码编辑器，减少页面卡顿）、流式渲染优化（默认开启，生成回答期间暂停消息区域动画与过渡，降低渲染开销）、禁用毛玻璃效果（可选，减少长对话滚动掉帧，会轻微改变页面视觉）。(#868)
...
- **适配 ChatGPT 新 DOM 结构** — 适配 ChatGPT 改版后的提问导航条与离屏空壳占位结构，修复刷新后大纲缺失完整用户提问列表，以及长对话导出静默跳过未挂载轮次、无法采集完整对话的问题。(#871, #872)
- **适配豆包新版输入框** — 适配豆包改版后的 TipTap 输入框，提示词插入、队列发送、生成中/停止检测和模型锁定在新输入区域及新版发送/停止按钮上恢复正常。(#867)
- **长对话与资产导出稳定性** — 适配 ChatGPT、豆包、DeepSeek、AI Studio 等虚拟滚动长对话导出，等待节点挂载、校验轮次连续性并定向补抓缺口，避免漏段、重复或乱序；检测到异常或超时将明确提示，ZIP 图片下载卡住时自动降级为外链，防止导出中断。(#865)
...
- **交互式导出弹窗** — 导出按钮与对话列表的导出入口现在统一打开导出选项弹窗，可选择导出格式（Markdown / HTML / JSON / TXT / 复制 Markdown）、切换思维链与对话序号、自定义 Markdown 分割线、打包 ZIP 或跳转分段导出；设置新增“导出前显示选项弹窗”开关，关闭后恢复一键直出。(#841)
...
- **适配豆包侧边栏新 DOM 结构** — 适配豆包新版侧边栏对话列表，修复同步到对话管理的对话标题重复三遍的问题。(#846)
...
- **Claude 新版页面适配** — 适配 Claude.ai 新版 DOM 结构，更新侧边栏对话列表、发送按钮、生成检测、禅模式、宽度调整和智能避让的选择器。（#834 @urzeye, #820 @crazyn2）
- **ChatGPT 新版页面适配** — 适配 ChatGPT 新版 DOM 结构：对话能被扫描到却无法加入对话管理、净化模式下无法移除「ChatGPT 也可能会犯错」提示、双击公式提示缺少可复制的源码；现已兼容新版侧边栏链接与公式结构。
...
- **隐藏对话自动恢复** — 修复部分历史对话在对话管理中不可见、即使重新同步也无法恢复的问题；升级后会自动把指向已删除文件夹的对话移回收件箱。
...
- **Claude 回复后的大纲刷新** — 修复已完成索引的 Claude 长对话在发送新问题后会重新全量扫描并滚动整段对话的问题；新回复现在只会增量更新已有大纲缓存。(#785)
...
- **Claude 页面结构适配** — 适配 Claude 更新后的对话页面 DOM 结构，确保大纲能够读取全部消息，并完整导出对话内容。(#781)

### 🐛 问题修复

- **Claude Canvas 文档导出** — 修复 Markdown 和剪贴板导出可能遗漏 Claude Canvas 文档正文的问题；现在会收集对话中的 Canvas 文档（包括虚拟列表中的文档），并在导出后恢复原先打开的 Canvas。
...
- **面板 Tab 排序** — 现在可以直接在面板中拖拽调整大纲、对话和提示词 Tab 的顺序，设置页会同步显示同一顺序，不必先打开选项页。(#764)
...
- **Kimi 对话同步** — 修复 Kimi 更新侧边栏布局后，新建或近期对话可能无法同步到对话面板的问题。(#759)
...
- **主面板工具栏与快捷按钮** — 统一大纲、提示词和对话面板的工具栏与搜索框样式，并调整快捷按钮反馈，让高频控制区在不同主题下更清楚。
...
- **自定义标签页标题格式** — 修复千问和 Z.ai 切换对话后可能复用已重命名的浏览器标签页标题的问题，避免使用包含 `{model}` 的自定义格式时出现标题或模型片段重复。
...
- **Gemini Canvas Markdown 操作** — Gemini Canvas 工具栏现在新增 Ophel 复制 Markdown 和下载 Markdown 按钮，可将 Canvas 代码导出为带代码块的 Markdown，不必执行完整对话导出。(#663)
...
- **AI Studio 移动端历史同步** — AI Studio 对话同步现在可以识别移动端 Library 卡片布局，移动端有历史记录时不再误提示找不到侧边栏对话。
- **对话同步加载** — 对话同步现在会使用各站点适配器自己的对话来源判断侧边栏历史是否加载完成，为慢速懒加载侧边栏等待更久，并在无法确认已完整加载时禁用同步删除；豆包和元宝也改为复用更安全的基类加载逻辑。(#639)
...
- **大纲复制** — 大纲工具栏现在可以复用导出流程，将当前选择层级的大纲复制为 Markdown 标题；虚拟滚动对话会先加载完整内容并在结束后恢复页面位置，再复制标题与用户提问。(#539)
- **对话快速打开** — 对话条目现在可通过右键菜单在新标签页打开，方便复用旧对话而不替换当前聊天。(#578)
...
- **ChatGPT 标签页状态** — 当侧边栏对话标题包含“Stop”或“停止”时，ChatGPT 空闲标签页不再误显示 `⏳`；生成状态检测现在只使用输入区内可见的停止按钮。(#630)
...
- **Gemini Canvas 代码导出** — Gemini 普通对话和分享页导出现在会打开 Canvas artifact，用不依赖界面语言的方式切换到代码标签页，并把生成的 Canvas 代码纳入 Markdown 导出。
...
- **Kimi 分享对话导出** — Kimi 分享页现在支持对话导出、大纲和滚动导航；导出会保留用户上传图片，并在 Markdown 与 zip 附件包中引用，同时纳入可见文件附件标签，确保用户提问与对应 AI 回复保持正确顺序。
- **通义千问附件与生成图片导出** — 通义千问 Markdown 导出现在会识别分享页新版问题/回答 DOM，保留用户提问中的图片和文件卡片，并在普通 Markdown 与 zip 附件包中导出 AI 生成图片，避免新版页面导出为空消息。
- **Qwen Studio 附件、生成图片与思维链导出** — Qwen Studio Markdown 导出现在会保留用户提问中的上传图片和完整文件卡片名称，在普通 Markdown 与 zip 附件包中导出 AI 生成图片，并可在开启「导出包含思维链」后导出普通对话和分享页展开后的思考内容。
- **元宝附件、生成图片与思维链导出** — 元宝 Markdown 导出现在会保留用户提问中的图片和文件，在普通 Markdown 与 zip 附件包中导出 AI 生成图片，并可在开启「导出包含思维链」后导出页面中可见的思考内容。
- **Z.ai 附件与思维链导出** — Z.ai 普通对话和分享页导出现在会保留用户上传的图片与文件，支持 Markdown 与 zip 附件包；同时会合并 API 与 DOM 中带签名 CDN 链接的重复附件，并在开启「导出包含思维链」后导出推理内容。
...
- **Gemini Deep Research 报告面板 Markdown 快捷操作** — 在 Gemini Deep Research 右侧报告面板工具栏新增复制 Markdown 和下载 Markdown 按钮，可直接导出当前报告内容，不影响普通对话导出流程。(#579)
- **Markdown 附件导出包** — 当站点适配器提供附件资源时，Markdown 导出现在可生成包含主文档、附件和 manifest 的 zip 包，并使用稳定的相对路径引用文件；生成的 Markdown 文档统一放入 `assets/documents/`、图片放入 `assets/images/`、用户上传附件放入 `assets/files/`，并按普通 Gemini 对话同样的方式解析用户上传文档。(#580)
- **Gemini Deep Research 导出** — Gemini Deep Research 现在可导出用户自己的应用内对话、对话分享页和生成报告分享页；报告侧栏关闭时会在导出期间自动打开并在结束后恢复，同时把用户提问中的上传文件标签纳入导出内容。(#573)
...
- **整行拖拽排序** — 提示词和对话文件夹现在可直接长按整行拖拽排序，与对话条目的交互保持一致，不再需要专门的拖动图标。
...
- **对话文件夹选择器图标间距** — 收紧同步文件夹选择框的内边距，并为文件夹 emoji 图标提供稳定的居中区域，避免在「对话」工具栏中看起来被裁切。
...
- **Kimi 对话跳转** — 修复在「对话」面板中点击 Kimi 侧边栏历史项时无法切换聊天的问题。现在会按解析出的对话 ID 精确匹配 Kimi 聊天链接，并在原生链接未挂载时使用 SPA 路由兜底。
...
- **阅读历史恢复范围收窄** — 阅读历史现在只会在用户自己的历史对话页保存和恢复位置，分享页、新对话页及站点内其它页面不再误触发；同时将对话页判断下沉到各站点适配器，便于后续维护。
- **对话同步清理** — 手动同步现在会移除同一站点、同一账号下已在原站删除的对话，并在 Gemini 同步前按需展开「最近」对话区域。
...
- **Gemini 适配器：兼容新版侧边栏结构** — 更新选择器与导航逻辑以支持新的 `gem-nav-list-item[data-test-id="conversation"]` 侧边栏项；新增滚动加载辅助函数，修复对话 ID/标题提取与跳转，恢复对 Gemini 侧边栏改版的兼容性。
...
- **支持将对话拖拽到文件夹**：在「对话」面板中，可直接把对话拖到任意文件夹完成移动。感谢 @t0ny-peng (#512)
...
- **修复 AI Studio 长对话导出有 turn 缺漏、反复滚动**：重写导出收集器，直接按 DOM 顺序遍历 `ms-chat-turn`，一次扫完所有用户提问、AI 回复（含附件、独立的「思考」turn）。
...
- **豆包删除对话确认异常**：适配豆包新版页面结构，修复删除对话时确认弹窗无法正常触发的问题（繁体界面同修）。
...
- **面板标签页图标略微放大**：大纲、对话、提示词图标尺寸从 14px 调整为 16px，更加清晰。
...
- **对话列表性能优化**：文件夹下大量对话展开不再卡顿，采用渐进式加载。(#369)
...
- **用户文档全面重写**：以中英双语重写全部用户指南，覆盖快速上手、面板概览、浮动按钮、智能大纲、对话管理、提示词库、增强功能、外观主题、备份同步、快捷键、常见问题等章节，内容更完整、更易上手。
...
- **Qwen Studio 站点命名统一**：将原 `chat.qwen.ai` 站点的外显名称统一调整为 `Qwen Studio`，同步覆盖适配器展示名、设置项、对话提示、README 与 Issue 模板等入口，减少新旧命名并存带来的理解成本。
...
- **豆包对话同步恢复**：适配豆包新版侧边栏 DOM 结构，恢复对话列表识别、当前对话定位与新建对话入口的同步能力，减少因官方页面改版导致的侧边栏功能失效。 (#342)
- **豆包大纲与导出结构适配**：切换到豆包新版聊天消息区 DOM，恢复大纲抽取、最新回复提取与对话导出等依赖消息结构的功能，避免官方改版后出现大纲为空或导出抓不到内容的问题。 (#342)
...
- **Gemini 对话图去水印误判修复**：调整 Gemini 去水印尺寸判定逻辑，在 `/app` 对话页识别到 preview 图片来源时优先使用 `48px` 水印配置，避免将小水印误按大图规则处理，导致原水印未去除、额外留下错误印记或亮边残影。 (#335)
...
- **导出标题被标签页重命名污染**：修复 AI Studio 导出文档一级标题误使用扩展改写后的浏览器标签标题的问题，现已优先使用原始对话标题；同时为 Grok 与通用导出链路补充保护，避免导出时被当前页面标题回写覆盖。
...
- **新对话快捷键兼容性**：修复 macOS 与部分站点上新建对话快捷键失效，及豆包误回退为整页刷新的问题。
...
- **DeepSeek 分享对话导出**：修复 `chat.deepseek.com/share/*` 分享页导出时误提示“请先打开要导出的对话”的问题，支持直接识别分享对话并完成导出。
...
- 修复 DeepSeek 删除后页面未刷新，以及豆包对话删除未同步到原网站的问题
...
- **Gemini MyStuff 新标签页打开**：在 Gemini「我的内容」页面为媒体卡片新增右上角“在新标签页中打开”入口，支持直接跳转回原始对话位置 (#185)
...
- 修复 Grok 对话同步误将命令面板 Actions 项当作对话的问题
...
- **对话同步结果诊断增强**：对话手动同步后新增“扫描/新增/更新”结果提示；当侧边栏未检测到对话时提供明确引导；并修正更新统计口径与元数据回填写入逻辑。by @joevalleyfield
- **DeepSeek 对话同步增强**：补齐对话重命名同步、链接与 CID 回写更新，并新增置顶对话识别，支持区分置顶/普通对话分组。
...
- **设置搜索漏项补齐**：补齐“快捷键位”“全局搜索”“对话同步删除”等此前无法被设置搜索命中的配置项，支持按名称与关键词定位。
...
- **ChatGPT Projects 导出失败**：修复在 `Projects` 对话中执行“导出 Markdown / 复制 Markdown”时，因项目内对话未同步到本地索引而报 `Conversation not found` 的问题。现在会从当前页面即时补齐对话元数据后再导出。
...
- **备份导出/导入数据结构错误**：修复了导出时丢失对话的 `lastUsedFolderId`（上次使用的文件夹）等辅助属性，以及导入时因判断条件错误导致包含 2 条以上对话的备份无法正确还原的问题。
...
- **Gemini 跨浏览器导入对话不可见**：将 Gemini 对话隔离标识从数字 `cid`（`/u/n`）升级为账号邮箱优先，并在插件升级后首次打开 Gemini 时自动迁移旧数据（数字 `cid` -> 邮箱 `cid`），兼容 `u/0` 导入到 `u/1` 等跨浏览器场景，避免对话/文件夹/标签导入后“看不见”。
...
- Gemini（普通版）导出支持自动展开对话内思维链，并在导出完成后恢复原折叠状态与阅读位置。
...
- 对话列表信息密度优化：标题与标签改为同一行展示，避免无标签对话出现第二行空白占位。
- 对话列表移除更新时间展示：不再单独占用一行显示时间，将可视空间优先让给标题与标签信息。
- 多标签折叠增强：当标签超出可显示数量时继续显示 `+N`，并支持悬浮查看完整标签列表（含标签颜色与名称）。
- 窄面板适配优化：在较窄宽度下自动收紧标签展示策略，保证标题可读性并降低信息拥挤感。
- 对话操作入口优化：右侧操作按钮采用更轻量的悬浮显隐交互，减少默认状态下对主信息的干扰。
...
- 对话管理新增“云端同步删除”能力，支持在对话 Tab 批量触发后同步删除站点云端对话。
...
- 修复对话批量删除统计中远端失败计数不准确的问题。
- 修复 Gemini / Gemini Enterprise UI 删除流程稳定性问题，覆盖菜单触发、删除点击与完成态判定。
- 修复 Gemini Enterprise 云端删除成功后本地对话未及时移除的问题。
...
- 全局搜索补全命中原因标签：对话（标题/文件夹/标签）、大纲（标题/类型/编号）、提示词（标题/分类/内容/ID）、设置（名称/关键词/ID/别名）。
...
- 新增 Search Everywhere 全局搜索弹窗，支持双击 Shift / Ctrl(Cmd)+K 触发，覆盖分类：全部、大纲、对话、提示词、设置。
...
- 搜索结果统一命中高亮样式；对话结果元信息单行化（站点/文件夹/标签）。
...
- 修复对话切换或大纲延迟加载后，全局搜索结果未及时刷新的问题。
- 修复提示词搜索回车行为在不同场景下反馈不一致的问题（无变量直接插入，含变量打开填写弹窗）。
- 修复对话文件夹名称 emoji 重复显示与下拉文本对齐问题，并抽离通用 `SelectDropdown` 组件用于统一下拉样式。

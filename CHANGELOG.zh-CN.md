...
- **Gemini Spark 会话同步** — Gemini Spark（gemini.google.com/spark）的任务会话现在可以同步进会话管理：同步按钮会读取 Spark 首页的近期对话和「所有任务」页的完整会话列表，置顶状态一并同步，点击已同步的会话可直接跳转到对应线程；在 Spark 页面同步不再提示“未在侧边栏检测到会话”。(#902)
- **跨站点会话搜索** — 全局搜索现在可以搜到其他已适配站点的会话标题，站外结果带来源站点图标和“站外”标注，点击后在新标签页打开对应会话。(#888)

### ✨ 功能优化

- **多入口站点的快捷访问** — 绑定多个域名的站点（如 Kimi 的 kimi.com 和 kimi.ai）在弹窗「快速访问」中现在只占一个格子，不再每个域名单独占格；格子副标题显示将打开的入口，右上角按钮可打开入口切换菜单（下方空间不足时自动向上展开），上次选择的入口会被记住。(#895)
- **悬浮提示显示优化** — 提示更轻巧，靠近屏幕边缘时自动翻转方向，点击、按下或按 Esc 立即关闭；连续扫过相邻按钮时提示即时出现、不再重复等待；快捷按钮提示朝屏幕中心展开；大纲、会话、分组、提示词链和已选提示词条等提示只在标题被截断时出现，悬停不再遮挡相邻内容；设置页主题切换仅在纯图标模式下显示提示。(#885)
- **更新提示方式** — 版本更新后不再自动弹出全屏更新日志。更新后右下角会出现一次性提醒条，面板 Logo 和收起状态的面板按钮会显示红点：悬停 Logo 可预览本次更新要点，点击查看完整更新日志，读完后红点消失；设置「关于」页的查看入口保留。(#887)
- **功能提示新增「适配中心」入口** — 大纲功能提示里的「适配中心」现在带高亮标识，点击可直接打开设置「适配中心」并定位到在线适配库。
- **顶栏拖拽提示优化** — 顶栏的穿透模式提示改为按住顶栏停留片刻才显示，点击、双击或拖动面板时不再闪出遮挡标题。

### 🐛 问题修复

- **Kimi 国际站支持** — 现在识别 Kimi 国际站 kimi.ai，面板不再显示"不支持"提示，大纲、导出、禅模式等 Kimi 功能在 kimi.ai 与 kimi.com 上均可用。(#890)
...
- **弹窗界面语言** — 修复扩展弹窗始终显示浏览器语言的问题，在设置中切换界面语言后，弹窗现在会正确显示所选语言，不再回退到浏览器语言。(#897)
